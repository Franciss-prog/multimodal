<script lang="ts">
	...
	let filtered = $derived(
		activeFilter === 'all' ? discussions : discussions.filter((d) => d.category === activeFilter)
	);
	...
	const filterColors: Record<FilterCategory, { active: string; inactive: string }> = {
		all: {
			active: 'bg-gradient-to-r from-[#5cb85c] to-[#3aa9be] text-white',
			inactive: 'text-gray-500 hover:text-gray-700'
		},
		air: {
			active: 'bg-gradient-to-r from-sky-500 to-indigo-500 text-white',
			inactive: 'text-gray-500 hover:text-sky-600'
		},
		water: {
		...
		land: {
			active: 'bg-gradient-to-r from-green-500 to-lime-500 text-white',
			inactive: 'text-gray-500 hover:text-green-600'
		},
		general: {
			active: 'bg-gradient-to-r from-slate-500 to-gray-600 text-white',
			inactive: 'text-gray-500 hover:text-gray-700'
		}
	};
</script>
...
<div class="mx-auto min-h-screen w-full max-w-screen-2xl px-6 py-10 md:px-10 lg:px-16">
	...
	<div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div>
			<h1
				class="bg-gradient-to-r from-emerald-700 to-cyan-700 bg-clip-text text-3xl font-black text-transparent lg:text-4xl"
			>
				Community Forums
			</h1>
		...
		<button
			onclick={openModal}
			class="flex w-fit items-center gap-2 rounded-full bg-gradient-to-r from-[#5cb85c] to-[#3aa9be] px-6 py-2.5 text-sm font-semibold text-white shadow-[0_16px_28px_-18px_rgba(56,189,248,0.9)] transition-all hover:brightness-105"
		>
...
{activeFilter === tab.key ? 'bg-white/25' : 'bg-gray-200 text-gray-500'}"
	...
	{#if filtered.length === 0}
		<div
			class="flex flex-col items-center justify-center rounded-3xl border border-emerald-100/70 bg-white/80 py-24 text-center shadow-sm backdrop-blur"
		>
			...
			<button
				onclick={openModal}
				class="mt-6 rounded-full bg-gradient-to-r from-[#5cb85c] to-[#3aa9be] px-6 py-2.5 text-sm font-semibold text-white hover:brightness-105"
			>
				Start a Discussion
			</button>
		</div>
		...
		<div class="flex flex-col gap-3">
			{#each filtered as d (d.id)}
				<button
					...
					class="group flex flex-col gap-3 rounded-2xl border border-emerald-100/80 bg-white/90 p-5 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md"
				>
					<!-- Top row: badge + status -->
					<div class="flex items-center justify-between gap-2">
						<span
							class="rounded-full px-3 py-1 text-xs font-semibold {categoryConfig[d.category]
								.badge}"
						>
...
{#if showModal}
	<div
		...
		role="presentation"
	>
		<div class="w-full max-w-lg rounded-2xl border border-emerald-100 bg-white p-6 shadow-xl">
			<div class="mb-5 flex items-center justify-between">
				<h2
					class="bg-gradient-to-r from-emerald-700 to-cyan-700 bg-clip-text text-xl font-black text-transparent"
				>
					New Discussion
				</h2>
				<button
			...
			>
				<!-- Name -->
				<div>
					<label class="mb-1 block text-xs font-semibold text-gray-600" for="author"
						>Your Name</label
					>
					<input
						id="author"
				...
				<!-- Category -->
				<div>
					<label class="mb-1 block text-xs font-semibold text-gray-600" for="category"
						>Category</label
					>
					<select
						id="category"
				...
				<!-- Body -->
				<div>
					<label class="mb-1 block text-xs font-semibold text-gray-600" for="body"
						>Description</label
					>
					<textarea
						id="body"
				...
				<div class="flex gap-3 pt-1">
					...
					<button
						type="submit"
						disabled={submitting}
						class="flex-1 rounded-xl bg-gradient-to-r from-[#5cb85c] to-[#3aa9be] py-2.5 text-sm font-semibold text-white transition-all hover:brightness-105 disabled:opacity-50"
					>
