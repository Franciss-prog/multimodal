<script lang="ts">
	...
	import { discussions, categoryConfig, timeAgo, type Category } from '$lib/stores/discussions.svelte';
	...
	let form = $state({ title: '', body: '', author: '', category: 'air' as Category });
	let formError = $state('');

	let filtered = $derived(
		activeFilter === 'all'
			? [...discussions.items]
			: discussions.items.filter((d) => d.category === activeFilter)
	);

	let counts = $derived({
		all: discussions.items.length,
		air: discussions.items.filter((d) => d.category === 'air').length,
		water: discussions.items.filter((d) => d.category === 'water').length,
		land: discussions.items.filter((d) => d.category === 'land').length,
		general: discussions.items.filter((d) => d.category === 'general').length
	});

	function openModal() {
		form = { title: '', body: '', author: '', category: 'air' };
		formError = '';
		showModal = true;
	}
	...
	function submitDiscussion() {
		if (!form.title.trim()) { formError = 'Please enter a title.'; return; }
		if (!form.body.trim()) { formError = 'Please describe your discussion.'; return; }
		if (!form.author.trim()) { formError = 'Please enter your name.'; return; }
	...
	>
		<div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
			...
			<div class="flex flex-col gap-4">
				...
				{#if formError}
					<p class="rounded-lg bg-red-50 px-4 py-2 text-sm text-red-500">{formError}</p>
				{/if}
				...
				<div class="flex gap-3 pt-1">
					...
					<button
						...
						class="flex-1 rounded-xl bg-[#5cb85c] py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#4aa04a]"
					>
						Post Discussion
					</button>
				</div>
